...
public class CityMarker extends SimplePointMarker {
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		pg.fill(255, 0, 0);
		float x1 = (float) (x - 0.5 * TRI_SIZE);
		float x2 = (float) (x + 0.5 * TRI_SIZE);
		float y1 = (float) (y + 1 / Math.sqrt(3.0) * 0.5 * TRI_SIZE);
		float y3 = (float) (y - 1 / Math.sqrt(3.0) * TRI_SIZE);
		pg.triangle(x1, y1, x2, y1, x, y3);
